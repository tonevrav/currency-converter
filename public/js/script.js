...
function addNewRate() {
    ...
    let found = false;
    for(let i = 0; i < allRates.length; i++) { // Check if the rate object already exists in the allRates array
...
function addElementsToHTML() {
    ...
    for(let i = 0; i < allRates.length; i++) {
        ...
        if(!addedFromCurrencies.includes(base)) { // Add the elements to the first section element
    ...
    convertFromEl.onchange = function() { // Dynamically add currencies to the second section element depending on which currency is selected in the first drop-down list
...
function convertCurrency(event) {
    ...
    resultMessageEl.innerText = `${amount} ${currencyFrom} will be ${result.toFixed(2)} ${currencyTo}`;
